...
public class Robot_36_Color extends BaseMode {
    ...
    Robot_36_Color(int sampleRate) {
        ...
        double scanLineSeconds = syncPulseSeconds + syncPorchSeconds + luminanceSeconds + separatorSeconds
                + porchSeconds + chrominanceSeconds;
    ...
    @Override
    public boolean decodeScanLine(PixelBuffer pixelBuffer, float[] scratchBuffer, float[] scanLineBuffer,
            int scopeBufferWidth, int syncPulseIndex, int scanLineSamples, float frequencyOffset) {
        ...
        for (int i = 0; i < horizontalPixels; ++i) {
            ...
            if (even) {
                pixelBuffer.pixels[i] = ColorConverter.RGB(scratchBuffer[luminancePos], 0,
                ...
                pixelBuffer.pixels[i + horizontalPixels] = ColorConverter
                        .YUV2RGB((oddYUV & 0x00ffff00) | (evenYUV & 0x000000ff));
            }
        }
